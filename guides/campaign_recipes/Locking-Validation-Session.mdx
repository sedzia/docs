...
| session.key `string` **optional**                                                 | Unique session identifier.                                                                                                                      | "session": `{"key": "ssn_yQGMTeKBSw8OOuFPwlBEjzGy8d8VA9Ts",<br>"type": "LOCK"}` |
| session.ttl_unit `string` **optional** Default value for session is 7 days | Defines the type of unit for session time. Allowed values: `DAYS`, `HOURS`, `MICROSECONDS`, `MILLISECONDS`, `MINUTES`, `NANOSECONDS`, `SECONDS` | "session": `{"type": "LOCK",<br>"ttl": 7,<br>"ttl_unit": "DAYS"}`               |
| session.ttl `number`**optional** Default value for session is 7 days      | Value for the period of time that the session is active. Units for this paremter are defines by session.ttl_unit                                | "session": `{"type": "LOCK",<br>"ttl": 7,<br>"ttl_unit": "DAYS"}`               |
